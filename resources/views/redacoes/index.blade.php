...
<div class="min-h-screen bg-gray-50 p-8">
    ...
    <label for="imagem_redacao" class="flex items-center justify-center gap-2 cursor-pointer w-full p-3 border border-dashed border-gray-400 rounded-lg bg-gray-100 hover:bg-gray-200 text-gray-700 font-medium transition">
    ...
    <div id="modal-apagar" class="fixed inset-0 bg-black bg-opacity-50 hidden flex justify-center items-center z-50">
        <div class="bg-white p-6 rounded-2xl shadow-md w-full max-w-md text-center relative">
...
<script>
    ...
    const fileNameDisplay = document.getElementById('imagem_redacao_filename');
    const labelText = document.getElementById('imagem_redacao_label_text');

    if (inputImagem) {
        inputImagem.addEventListener('change', function () {
            const fileName = inputImagem.files.length > 0 ? inputImagem.files[0].name : 'Selecionar Arquivo';
            labelText.textContent = 'Arquivo Selecionado';
    ...
    function confirmarExclusao() {
        if (idParaApagar) {
            document.getElementById(`form-apagar-${idParaApagar}`).submit();
        }
    }
</script>
@endsection
