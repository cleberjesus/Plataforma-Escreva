                ...
                <form action="{{ route('redacoes.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                    ...
                    <div>
                        ...
                        <select id="modo_envio" name="modo_envio" class="w-full p-3 border border-gray-300 rounded-lg" required>
                            <option value="digitado" {{ old('modo_envio') == 'digitado' ? 'selected' : '' }}>Digitado</option>
                            <option value="imagem" {{ old('modo_envio') == 'imagem' ? 'selected' : '' }}>Imagem</option>
                        </select>
                    </div>

                    <div id="texto_redacao_div">
                        <label for="texto_redacao" class="block text-gray-700 mb-2 font-semibold">Texto da Redação</label>
                        <textarea id="texto_redacao" name="texto_redacao" rows="5" class="w-full p-3 border border-gray-300 rounded-lg">{{ old('texto_redacao') }}</textarea>
                    </div>
                    ...
                    <div class="flex flex-col sm:flex-row gap-4 justify-center items-center">
                        ...
                        <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 rounded-lg font-semibold w-full sm:w-auto">Salvar Redação</button>
                    </div>
                </form>
            </div>
        </div>
